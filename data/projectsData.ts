...
export const projectsData: Project[] = [
  {
    ...
    coverImage: "/protpolio/1-agrishield/1.png",
    images: [
      { url: "/protpolio/1-agrishield/1.png", caption: "대시보드 메인 화면: 기존 프로젝트 목록을 관리하고 새로운 농경지 병해충 분석을 시작할 수 있는 진입점입니다." },
      { url: "/protpolio/1-agrishield/2.png", caption: "신규 분석 설정: 대상지의 주소, 면적 등의 상세 정보를 입력하고 드론으로 촬영된 원본 이미지 및 로그 데이터를 업로드하여 AI 분석 파이프라인을 가동합니다." },
      { url: "/protpolio/1-agrishield/3.png", caption: "프로젝트 개요: 분석 처리 상태(진행 중, 완료, 실패)를 직관적인 카드 뷰로 모니터링하며, 현재까지 탐지된 주요 병해충 유형을 요약하여 보여줍니다." },
      { url: "/protpolio/1-agrishield/6.png", caption: "상세 탐지 목록: AI 모델이 탐지한 병해충 증상을 목록화하여 제공합니다. 각 항목을 클릭하면 해당 이미지가 확대되며, 모델의 구체적인 진단 내역을 확인할 수 있습니다." },
      { url: "/protpolio/1-agrishield/7.png", caption: "지도 시각화 및 리포트 연동: 분석된 위치 정보를 바탕으로 지도 위에 병해충 발생 지점을 마커로 시각화합니다. 모든 분석 데이터를 종합하여 최종 PDF 보고서를 생성합니다." },
      { url: "/protpolio/1-agrishield/8.jpg", caption: "자동 생성 PDF 리포트 예시 (1): Vision LLM 기반으로 자동 생성된 PDF 리포트의 실제 출력물입니다. 촬영 위치 데이터와 종합적인 분석 개요가 문서화되어 관공서 제출용으로 활용됩니다." },
      { url: "/protpolio/1-agrishield/9.jpg", caption: "자동 생성 PDF 리포트 예시 (2): 세부적인 개별 병해충 탐지 결과 및 원본 증상 사진이 포함된 페이지입니다. 실무자가 현장 상황을 정확히 파악할 수 있도록 AI 판독 결과가 일목요연하게 정리되어 있습니다." }
    ],
    keyFeatures: [
  ...
  },
  {
    ...
    coverImage: "/protpolio/2-ivycode-cli/Ivycodecli-1-2.png",
    images: [
      { url: "/protpolio/2-ivycode-cli/Ivycodecli-1-1.png", caption: "CLI 에이전트 초기화 및 툴콜(Tool Call) 처리: 사용자의 자연어 프롬프트를 인식하여 필요한 도구(파일 시스템 접근, 터미널 명령어 실행 등)를 자동으로 기획하고 수행하는 과정입니다." },
      { url: "/protpolio/2-ivycode-cli/Ivycodecli-1-3.png", caption: "실시간 스트리밍 코드 생성: LLM의 응답을 터미널 상에 실시간 스트리밍으로 부드럽게 출력하며, 생성된 코드를 즉시 프로젝트 파일에 반영하거나 명령어를 백그라운드에서 실행합니다." }
    ],
    keyFeatures: [
  ...
  },
  {
    ...
    coverImage: "/protpolio/3-ivycode-ext/Ivycode-ext-1-1.png",
    images: [
      { url: "/protpolio/3-ivycode-ext/Ivycode-ext-1-2.png", caption: "VS Code 네이티브 인터페이스: 개발 환경에 완벽히 통합된 사이드바 챗봇 뷰어입니다. 자연어 채팅을 통해 에디터 조작, 코드 리뷰, 버그 수정 등을 에이전트에게 지시할 수 있습니다." },
      { url: "/protpolio/3-ivycode-ext/Ivycode-ext-1-3.png", caption: "Context-Aware 자동 완성 및 분석: 현재 열려있는 파일의 전체 코드, 커서 위치, 그리고 드래그한 블록을 에이전트가 실시간 문맥으로 파악하여 가장 적절하고 정확한 코드 수정안을 제시합니다." },
      { url: "/protpolio/3-ivycode-ext/Ivycode-ext-1-4.png", caption: "인라인 Diff 뷰어 연동: 에이전트가 제안한 코드 변경 사항을 적용하기 전, 기존 원본 코드와 한눈에 비교할 수 있는 네이티브 Diff 뷰어를 제공하여 개발자가 안전하게 코드를 반영할 수 있도록 지원합니다." }
    ],
    keyFeatures: [
  ...
  },
  {
    ...
    coverImage: "/protpolio/4-githubcodereview/review-1.png",
    images: [
      { url: "/protpolio/4-githubcodereview/review-2.png", caption: "자동화된 PR 코드 리뷰 봇: Github 또는 Gitea 저장소에 Pull Request가 생성되면 Webhook을 통해 즉시 코드 변경점(Git Diff)을 분석합니다. 버그 발생 가능성, 보안 취약점, 최적화 방안을 마크다운 형태의 상세한 코멘트로 자동 등록해주는 실제 동작 화면입니다." }
    ],
    keyFeatures: [
  ...
  },
  {
    ...
    coverImage: "/protpolio/6-home-page/Clevi1-1.png",
    images: [
      { url: "/protpolio/6-home-page/Clevi1-2.png", caption: "Next.js 기반 고성능 랜딩 페이지: 회사의 비전과 연혁을 세련된 UI로 담아낸 메인 화면입니다. Server-Side Rendering(SSR)을 적극적으로 활용하여 검색 엔진 최적화(SEO)와 압도적으로 빠른 초기 로딩 속도를 달성했습니다." },
      { url: "/protpolio/6-home-page/Cs-1.png", caption: "반응형 서비스 포트폴리오 뷰: 모바일, 태블릿, 데스크탑 등 다양한 디바이스 화면 크기에 완벽하게 대응하는 반응형 그리드 레이아웃을 구축하여, 주요 서비스와 포트폴리오를 방문자에게 직관적으로 소개합니다." }
    ],
    keyFeatures: [
